 ...
 */
    ...
    (() => {
        ...
        function slidesPlugin(activeSlide = -1) {
            const sections = document.querySelectorAll(".offers__offer");
            if (activeSlide >= 0 && activeSlide < sections.length) sections[activeSlide].classList.add("offers__offer-active");
            for (const section of sections) section.addEventListener("click", (() => {
                if (section.classList.contains("offers__offer-active")) section.classList.remove("offers__offer-active"); else section.classList.add("offers__offer-active");
                clearOtherActiveClasses(section);
            }));
            function clearOtherActiveClasses(clickedSection) {
                sections.forEach((section => {
                    if (section !== clickedSection) section.classList.remove("offers__offer-active");
                    changeTitleColor(section);
                }));
            }
            function changeTitleColor(section) {
                const title = section.querySelector(".offers__title");
                if (section.classList.contains("offers__offer-active")) title.style.color = "$orangeColor"; else title.style.color = "$mainColor";
            }
        }
        slidesPlugin();
        window["FLS"] = true;
